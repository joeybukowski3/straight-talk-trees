...
- Each indexable route has unique metadata, an absolute `https://bukowskitrees.com` canonical, Open Graph and Twitter metadata, and `index, follow`.
...
- `robots.txt` permits normal crawling and references `https://bukowskitrees.com/sitemap.xml`.
...
- Verify `bukowskitrees.com` as an authorized sending domain in Resend.
...
- Verify the site in Google Search Console and submit `https://bukowskitrees.com/sitemap.xml`.
